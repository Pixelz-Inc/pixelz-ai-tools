...
def remove_background(imagePath: str, backgroundColor: str = None, transparentBackground: bool = True,
    ...
    try:
        ...
        return json.dumps(requests.post(f"{BASE_URL}/images/remove-background", json=payload, headers=api_headers(token, sync), timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'remove_background failed', {'error': str(e)})
...
def color_matching(imagePath: str, colorMarkers: list, callbackUrl: str = None, sync: bool = False) -> str:
    ...
    try:
        ...
        return json.dumps(requests.post(f"{BASE_URL}/images/color-matching", json=payload, headers=api_headers(token, sync), timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'color_matching failed', {'error': str(e)})
        return f"Error: {str(e)}"
...
def create_mask(imagePath: str, featherWidth: float = None, trimapUrl: str = None, callbackUrl: str = None,
    ...
    try:
        ...
        return json.dumps(requests.post(f"{BASE_URL}/images/create-mask", json=payload, headers=api_headers(token, sync), timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'create_mask failed', {'error': str(e)})
        return f"Error: {str(e)}"
...
def create_trimap(imagePath: str, callbackUrl: str = None, sync: bool = False) -> str:
    ...
    try:
        ...
        return json.dumps(requests.post(f"{BASE_URL}/images/create-trimap", json=payload, headers=api_headers(token, sync), timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'create_trimap failed', {'error': str(e)})
        return f"Error: {str(e)}"
...
def model_crop(imagePath: str, topCropLocation: str = None, bottomCropLocation: str = None,
    ...
    try:
        ...
        return json.dumps(requests.post(f"{BASE_URL}/images/model-crop", json=payload, headers=api_headers(token, sync), timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'model_crop failed', {'error': str(e)})
        return f"Error: {str(e)}"
...
def get_job_status(jobId: str) -> str:
    ...
    try:
        ...
        return json.dumps(requests.get(f"{BASE_URL}/images/jobs/{jobId}/status", headers={'Authorization': f'Bearer {get_token()}'}, timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'get_job_status failed', {'error': str(e)})
        return f"Error: {str(e)}"
...
def get_webhook_public_key() -> str:
    ...
    try:
        ...
        return json.dumps(requests.get(f"{BASE_URL}/webhook/public-keys", headers={'Authorization': f'Bearer {get_token()}'}, timeout=REQUEST_TIMEOUT).json(), indent=2)
    except Exception as e:
        log('error', 'get_webhook_public_key failed', {'error': str(e)})
        return f"Error: {str(e)}"
